...
- Player holds last_drawn and must choose:
  ...
  - attempt_match (2-4 slots): if all selected cards match each other, discard
    those cards and discard last_drawn; otherwise reveal chosen slots
    (public_known) and add last_drawn to hand (hand +1). End turn.
