(() => {
    ...
    let nextId = 0;
    const selector = "[data-rebel-tip]";

    function installStyles() {
        if (document.getElementById("rebelHintStyles")) return;
        const style = document.createElement("style");
        style.id = "rebelHintStyles";
        style.textContent = `
            #rebelPrincessPanel [data-rebel-tip] { cursor: help; }
            #rebelPrincessPanel [data-rebel-tip]:focus-visible {
                outline: 3px solid #2563eb;
                outline-offset: 3px;
                border-radius: 6px;
            }
            body.rebel-explain-mode #rebelPrincessPanel [data-rebel-tip] {
            ...
            .rebel-instant-hint {
                ...
                box-shadow: 0 8px 30px #452c4233;
                margin-top: env(safe-area-inset-top, 0px);
                margin-bottom: env(safe-area-inset-bottom, 0px);
            }
            .rebel-instant-hint[hidden] { display: none; }
            .rebel-instant-hint.rebel-hint-banner {
                pointer-events: none;
                border-left: 4px solid #a14475;
            }
        `;
    ...
    /**
     * Dedicated info targets only: place data-rebel-tip on resource badges or
     ...
     */
    window.createRebelHints = function ({ panel, isExplaining = () => false, onExplain = () => {} } = {}) {
        if (!panel) return { refresh() {}, hide() {}, destroy() {} };
        controllers.get(panel)?.destroy();
        installStyles();

        const id = `rebelInstantHint${++nextId}`;
        const popup = document.createElement("div");
        popup.id = id;
        popup.className = "rebel-instant-hint";
        popup.hidden = true;
        ...
        function show(element, nextMode) {
            ...
            const text = element.dataset.rebelTip?.trim();
            if (!text) return;
            dismissPopup();
            target = element;
            mode = nextMode;
            popup.classList.toggle("rebel-hint-banner", mode === "banner");
        ...
        function refresh() {
            ...
            panel.querySelectorAll(selector).forEach(element => {
                ...
                if (!suppliedLabels.get(element)) element.setAttribute("aria-label", element.dataset.rebelTip || "Info");
            });
            if (target) {
                const text = target.dataset.rebelTip?.trim();
                if (!text) hide();
                else if (popup.textContent !== text) {
                    popup.textContent = text;
                    position();
                }
            }
