...
fastapi_app.mount(
    "/static/carcassonne",
    StaticFiles(directory="assets/task36_tiles_svg"),
...
def _schedule_six_nimmt_timeout(room: Room) -> None:
    if room.game_type != "six_nimmt" or room.status != "in_game" or not room.game_state:
        room.six_nimmt_timeout_at_ms = None
        return
    state = room.game_state
    ...
    if at_ms <= 0:
        room.six_nimmt_timeout_at_ms = None
        return
    if room.six_nimmt_timeout_at_ms == at_ms:
        return
    room.six_nimmt_timeout_at_ms = at_ms
