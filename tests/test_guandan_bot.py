...
class GuandanBotBombAvoidanceTests(unittest.TestCase):
    ...
    def _pick_card_ids(self, hand, labels):
        remaining = list(hand)
        picked = []
        for label in labels:
            for idx, card in enumerate(remaining):
                if guandan._card_label(card) == label:
                    picked.append(remaining.pop(idx)["id"])
                    break
            else:
                raise AssertionError(f"missing card {label}")
        return picked
    ...
    def _make_midgame_orphan_cleanup_state(self):
        players = [
            {"player_id": "ccc", "name": "ccc", "seat": 0, "is_bot": False},
            {"player_id": "bot3", "name": "Bot 3", "seat": 1, "is_bot": True},
            {"player_id": "zara", "name": "Zara", "seat": 2, "is_bot": False},
        ...
        state["players"]["zara"]["hand"] = self._pick_labels(
            deck,
            [
                "♥️2", "♠️A", "♥️A", "♠️J", "♥️J", "♣️J", "♦️J",
                "♥️10", "♣️10", "♥️8", "♣️8", "♦️8", "♠️7",
                "♠️6", "♣️5", "♦️4", "♠️3", "♣️3", "♣️3", "♦️3",
            ],
        )
        state["players"]["bot4"]["hand"] = self._pick_labels(
            deck,
    ...
    def _make_full_house_tail_cleanup_state(self):
        players = [
            {"player_id": "ccc", "name": "ccc", "seat": 0, "is_bot": False},
            {"player_id": "bot3", "name": "Bot 3", "seat": 1, "is_bot": True},
            {"player_id": "zara", "name": "Zara", "seat": 2, "is_bot": False},
        ...
        state["players"]["ccc"]["hand"] = self._pick_labels(
            deck,
            ["♥️2", "♠️9", "♦️9", "♦️9", "♣️7", "♣️7", "♠️5", "♥️5", "♥️5"],
        )
        state["players"]["bot3"]["hand"] = self._pick_labels(
            deck,
            ["♥️2", "♥️A", "♥️Q", "♠️J", "♣️J", "♥️9", "♥️8", "♥️8", "♣️8"],
        )
        state["players"]["zara"]["hand"] = self._pick_labels(
            deck,
            ["♠️A", "♥️A", "♣️A", "♦️A", "♠️K", "♣️K", "♠️Q", "♦️Q", "♥️J", "♥️J"],
        )
        state["players"]["bot4"]["hand"] = self._pick_labels(
            deck,
            [
                "♠️2", "♦️Q", "♠️7", "♥️7", "♦️7", "♦️7", "♠️6",
                "♦️6", "♠️4", "♥️4", "♥️4", "♣️4", "♦️4",
            ],
        )
        state["seen_cards"] = [card["id"] for card in deck]
    ...
    def test_save_62f633_midgame_sheds_orphan_before_intact_full_houses(self):
        state = self._make_midgame_orphan_cleanup_state()
        hand = state["players"]["bot3"]["hand"]
        single_eight = [
            card["id"] for card in hand if guandan._card_label(card) == "♠️8"
        ]
        low_full_house = self._pick_card_ids(
            hand,
            ["♠️4", "♣️4", "♣️4", "♠️6", "♦️6"],
        )

        single_components = guandan._bot_score_components(
            state,
            "bot3",
            single_eight,
            depth=2,
        )
        full_house_components = guandan._bot_score_components(
            state,
            "bot3",
            low_full_house,
            depth=2,
        ...
        self.assertGreater(single_components.get("shed_low_single", 0.0), 0.0)
        self.assertGreater(single_components["total"], full_house_components["total"])
    ...
    def test_low_single_cleanup_does_not_break_an_existing_straight(self):
        deck = guandan._full_deck()
        hand = self._pick_labels(
            deck,
            ["🃏B", "♠️A", "♣️A", "♠️3", "♠️4", "♠️5", "♠️6", "♠️7"],
        )
        low_three = [
            card["id"] for card in hand if guandan._card_label(card) == "♠️3"
        ]
        ...
        self.assertEqual(
            guandan._lead_low_single_escape_bonus(hand, low_three, level_rank=2),
            0.0,
        )

    def test_save_79fe86_sheds_single_before_creating_a_four_single_tail(self):
        state = self._make_full_house_tail_cleanup_state()
        hand = state["players"]["bot3"]["hand"]
        single_nine = self._pick_card_ids(hand, ["♥️9"])
        full_house = self._pick_card_ids(
            hand,
            ["♥️8", "♥️8", "♣️8", "♠️J", "♣️J"],
        )
    ...
    def test_rollout_policy_uses_lightweight_action(self):
        state, big = self._make_state()
        with mock.patch(
            "game.guandan_ai._heuristic_best_action",
            side_effect=AssertionError("rollout should use the lightweight policy"),
        ):
    ...
    def test_lead_option_score_is_reused_within_one_decision(self):
        ...
        cards = [state["players"]["bot"]["hand"][0]["id"]]

        with mock.patch(
            "game.guandan_ai._compute_lead_option_score",
            wraps=guandan._guandan_ai._compute_lead_option_score,
        ) as compute:
            first = guandan._lead_option_score(state, "bot", cards)
            second = guandan._lead_option_score(state, "bot", cards)
    ...
    def test_mcts_reuses_root_particles_and_halves_weak_candidates(self):
        players = [
            {"player_id": "bot", "name": "Bot", "seat": 0, "is_bot": True},
            {"player_id": "opp", "name": "Opp", "seat": 1, "is_bot": False},
            {"player_id": "mate", "name": "Mate", "seat": 2, "is_bot": False},
        ...
        state["phase"] = "playing"
        state["current_turn"] = "bot"
        state["config"]["bot_mcts_early_stop_gap"] = 999.0
        state["config"]["bot_mcts_successive_halving_min_rounds"] = 2

        actions = [
            {"type": "play", "card_ids": [card_id]}
            for card_id in (11, 22, 33, 44)
        ]
        particle_counter = [0]
        ...
        self.assertEqual(picked, actions[0])
        self.assertEqual(branches_by_particle[1], {(11,), (22,), (33,), (44,)})
        self.assertEqual(branches_by_particle[2], {(11,), (22,), (33,), (44,)})
        self.assertLess(particle_counter[0], reply_tree.call_count)
    ...
    def test_mcts_gate_uses_heuristic_score_gap(self):
        state, _big = self._make_state()
        state["current_trick"]["combo"] = {"type": "full_house", "rank_value": 55, "size": 5}
        actions = [
            {"type": "play", "card_ids": [11, 12, 13, 14, 15]},
            {"type": "play", "card_ids": [21, 22, 23, 24, 25]},
            {"type": "play", "card_ids": [31, 32, 33, 34, 35]},
            {"type": "pass"},
        ]

        with mock.patch("game.guandan_ai._candidate_actions", return_value=actions):
            with mock.patch("game.guandan_ai._filter_overbomb_actions", side_effect=lambda _s, _p, acts: acts):
                with mock.patch("game.guandan_ai._action_combo", return_value={"type": "pair"}):
                    with mock.patch(
                        "game.guandan_ai._get_cached_heuristic_scored_candidates",
                        return_value=[(actions[0]["card_ids"], 30.0, {}), (None, 5.0, {})],
                    ):
                        self.assertFalse(guandan._should_use_mcts(state, "bot", width=4))
                    with mock.patch(
                        "game.guandan_ai._get_cached_heuristic_scored_candidates",
                        return_value=[(actions[0]["card_ids"], 10.0, {}), (None, 9.5, {})],
                    ):
                        self.assertTrue(guandan._should_use_mcts(state, "bot", width=4))
    ...
    def test_pair_response_blocks_next_two_card_opponent_before_teammate(self):
        players = [
            {"player_id": "calvin", "name": "calvin", "seat": 0, "is_bot": False},
            {"player_id": "bot3", "name": "Bot 3", "seat": 1, "is_bot": True},
            {"player_id": "zhu", "name": "zhu", "seat": 2, "is_bot": False},
        ...
        deck = guandan._full_deck()
        lead = self._pick_labels(deck, ["♦️6", "♦️6"])
        ...
        state["players"]["bot3"]["hand"] = deck[:11]
        del deck[:11]
        state["players"]["zhu"]["hand"] = deck[:8]
        del deck[:8]
        state["current_trick"] = {
            "player_id": "zhu",
            "cards": [card["id"] for card in lead],
        ...
        hand_by_label = {guandan._card_label(card): card for card in bot_hand}
        pair_k = [hand_by_label[label]["id"] for label in ("♦️K", "♠️K")]
        pass_score = guandan._bot_score_components(state, "bot4", None, depth=4)
        pair_score = guandan._bot_score_components(state, "bot4", pair_k, depth=4)

        self.assertLess(pass_score.get("pass_next_enemy_closeout", 0.0), -30.0)
        self.assertNotIn("strategic_enemy_pass", pass_score)
        self.assertGreater(pair_score.get("block_next_closeout", 0.0), 20.0)
        self.assertGreater(pair_score["total"], pass_score["total"])
        ...
        with mock.patch(
            "game.guandan_ai._get_cached_heuristic_scored_candidates",
            return_value=stale_scores,
        ):
            accepts_block = guandan._guandan_ai.call(
                guandan,
                "_should_accept_mcts_override",
                state,
                "bot4",
                {"type": "pass"},
                {"type": "play", "card_ids": pair_k},
                4,
            )
        self.assertTrue(accepts_block)
        rejects_pass = guandan._guandan_ai.call(
            guandan,
            "_should_accept_mcts_override",
            state,
            "bot4",
        ...
        self.assertEqual(chosen_combo.get("type"), "pair")
        self.assertEqual(chosen_combo.get("rank"), 13)

    def test_double_down_single_defers_to_immediate_teammate(self):
        players = [
            {"player_id": "calvin", "name": "calvin", "seat": 0, "is_bot": False},
            {"player_id": "bot3", "name": "Bot 3", "seat": 1, "is_bot": True},
            {"player_id": "zhu", "name": "zhu", "seat": 2, "is_bot": False},
        ...
        deck = guandan._full_deck()
        lead = self._pick_labels(deck, ["♣️A"])
        bot_hand = self._pick_labels(
            deck,
            ["♥️2", "♦️8", "♥️8", "♦️8", "♠️4", "♠️4", "♣️3", "♦️3"],
        )
        state["players"]["bot3"]["hand"] = bot_hand
        state["players"]["calvin"]["hand"] = self._pick_labels(
            deck,
        ...
        raw_pressure = guandan._guandan_ai.call(
            guandan,
            "_enemy_double_down_closeout_pressure",
            state,
            "bot3",
        )
        backstop = guandan._guandan_ai.call(
            guandan,
            "_immediate_teammate_backstop_confidence",
            state,
            "bot3",
            current_combo,
        )
        heart_two = [
            next(card["id"] for card in bot_hand if guandan._card_label(card) == "♥️2")
        ]
        pass_score = guandan._bot_score_components(state, "bot3", None, depth=4)
        heart_score = guandan._bot_score_components(state, "bot3", heart_two, depth=4)

        self.assertGreater(backstop, 0.7)
        self.assertLess(
            abs(pass_score.get("pass_enemy_double_down_threat", 0.0)),
            raw_pressure * 0.3,
        )
        self.assertLess(
            heart_score.get("deny_enemy_double_down", 0.0),
            raw_pressure * 0.3,
        )
        self.assertGreater(pass_score["total"], heart_score["total"])
        self.assertEqual(guandan.GuandanGame.bot_move(state, "bot3"), {"type": "pass"})

    def test_grouped_hand_with_one_bomb_does_not_keep_leading_singles(self):
        players = [
            {"player_id": "calvin", "name": "calvin", "seat": 0, "is_bot": False},
            {"player_id": "bot3", "name": "Bot 3", "seat": 1, "is_bot": True},
            {"player_id": "zhu", "name": "zhu", "seat": 2, "is_bot": False},
        ...
        bot_hand = self._pick_labels(
            ...
            [
                ...
                "♦️9", "♠️9", "♥️9", "♦️9",
                "♦️8", "♥️8", "♦️8",
                "♣️6", "♠️6", "♥️5", "♠️4", "♠️4", "♣️3", "♦️3",
            ],
        )
        state["players"]["bot3"]["hand"] = bot_hand
        for player_id, count in (("calvin", 27), ("zhu", 22), ("bot4", 27)):
            state["players"][player_id]["hand"] = deck[:count]
            del deck[:count]
        state["seen_cards"] = [card["id"] for card in deck]
        self._assert_consistent_card_zones(state)
        ...
        hand_by_label = {guandan._card_label(card): card for card in bot_hand}
        single_ten = [hand_by_label["♠️10"]["id"]]
        pair_three = [hand_by_label[label]["id"] for label in ("♣️3", "♦️3")]
        single_score = guandan._bot_score_components(
            state,
            "bot3",
            single_ten,
            depth=4,
        )
        pair_score = guandan._bot_score_components(
            state,
            "bot3",
            pair_three,
            depth=4,
        ...
        self.assertLess(single_score.get("lead_control_probe", 0.0), 2.0)
        self.assertLess(single_score.get("lead_retake_stock", 0.0), 1.0)
        self.assertGreater(pair_score["total"], single_score["total"])
        ...
        self.assertEqual(chosen_labels, sorted(["♣️3", "♦️3"]))
