...
ANIMAL_ABILITY_DEFINITIONS = {
    ...
    "venom": _ability("毒液", "Venom", "immediate", "opponent_action_debuff", "每名吸引力至少5且在你之前的玩家，在其槽位1至N的行动牌上各放1枚毒液标记。", parameters=["tokens_per_target"], edge_cases_zh=["指定行动牌已有毒液时丢弃新标记，不改放到其他牌。", "使用中毒行动时移除其毒液；若回合结束仍未移除任何毒液，须支付2金币。", "把中毒行动牌用于获得X标记的替代行动也会移除毒液（已持有5枚X而无法执行时除外）。", "休息时移除全部毒液。"]),
...
DATA_CORRECTIONS = [
    {"card_id": "475", "field": "abilities.hypnosis", "value": "immediate", "note_zh": "催眠立即确定目标；按设计团队澄清，实际借用行动仍在当前行动结束后执行。"},
    {"card_id": "485", "field": "abilities.hypnosis", "value": "immediate", "note_zh": "催眠立即确定目标；按设计团队澄清，实际借用行动仍在当前行动结束后执行。"},
