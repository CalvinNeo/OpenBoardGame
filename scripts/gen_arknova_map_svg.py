...
def build_cells(config):
    ...
    cells = []
    explicit_cells = config.get("cells")
    if explicit_cells:
        for explicit in explicit_cells:
            cell = dict(explicit)
            cell["terrain_hint"] = explicit.get(
                "terrain_hint",
                terrain_map.get(explicit["id"], "land"),
            )
            build_requirement = explicit.get(
                "build_requirement",
                build_requirement_map.get(explicit["id"]),
            )
            if build_requirement is not None:
                cell["build_requirement"] = build_requirement
            elif "build_requirement" in cell:
                del cell["build_requirement"]
            cells.append(cell)
    else:
            ...
            cells.append(
                {
                    ...
                    "x": extra["x"],
                    "y": extra["y"],
                    "terrain_hint": extra.get(
                        "terrain_hint", terrain_map.get(extra["id"], "land")
                    ),
    ...
    threshold = (
        config.get("cell_extraction", {}).get("neighbor_distance_threshold")
        or config.get("grid_fit", {}).get("neighbor_distance_threshold", 220)
    )
...
def render_svg(config, cells, output_path):
    ...
    dense = len(cells) > 80
    cell_radius = 15 if dense else 26
    cell_font_size = 16 if dense else 26
    bonus_font_size = 18 if dense else 22
    ...
    lines = [
        ...
        "      .title { font-size: 42px; font-weight: 800; fill: #2d241a; }",
        "      .subtitle { font-size: 22px; fill: #5b5147; }",
        f"      .cell-label {{ font-size: {cell_font_size}px; font-weight: 800; fill: #ffffff; text-anchor: middle; dominant-baseline: middle; }}",
        f"      .bonus-label {{ font-size: {bonus_font_size}px; font-weight: 800; fill: #231a12; text-anchor: middle; dominant-baseline: middle; }}",
    ...
    for cell in cells:
        ...
        lines.append(
            f'  <circle cx="{cell["x"]}" cy="{cell["y"]}" r="{cell_radius}" fill="{fill}" fill-opacity="0.78" stroke="#fff" stroke-width="3" />'
        )
    ...
    for bonus in config.get("cover_bonuses", []):
        ...
        y = bonus["y"]
        lines.append(
            f'  <rect x="{x - 40}" y="{y - 58}" width="80" height="30" rx="10" fill="rgba(255,247,210,0.94)" stroke="#8a6a1f" stroke-width="3" />'
        )
        lines.append(f'  <text x="{x}" y="{y - 43}" class="bonus-label">{bonus["code"]}</text>')

    for requirement in config.get("build_ii_cells", []):
        x = requirement["x"]
        y = requirement["y"]
        lines.append(
            f'  <rect x="{x - 40}" y="{y - 58}" width="80" height="30" rx="10" fill="rgba(255,235,220,0.94)" stroke="#b25a2b" stroke-width="3" />'
        )
        lines.append(f'  <text x="{x}" y="{y - 43}" class="bonus-label">{requirement["code"]}</text>')
    ...
    lines.extend(
        [
            ...
            f'  <text x="42" y="{legend_y - 6}" class="title">Map 0 Annotated</text>',
            f'  <text x="42" y="{legend_y + 22}" class="subtitle">Explicit per-hex cell centers, terrain hints, cover bonuses, Build II cells, and left-track rewards.</text>',
            ...
            f'  <text x="42" y="{legend_y + 148}" class="small">Cells are now explicit coordinates rather than a single regular-row fit.</text>',
        ]
    )
